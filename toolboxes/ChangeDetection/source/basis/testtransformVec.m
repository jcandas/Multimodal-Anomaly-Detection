function parameters = testtransformVec(parameters,methods)

% Test vectorial multilevel transform 
...
fprintf('\n');
fprintf('Vanishing Test ---------------------------------\n');

tic; 
for n = 1 : numvecs
    [coeff, levelcoeff, dcoeffs, ccoeffs] = hbtrans(Q(:,n,:), multileveltree, ind, datacell, datalevel);
    Qv = invhbtrans(dcoeffs, ccoeffs, multileveltree, ind, datacell, datalevel, numofpoints, dimdata);
    polyerror(n) = norm(squeeze(Q(:,n,:)) - squeeze(Qv), 2) / norm(squeeze(Q(:,n,:)));
    wavecoeffnorm(n) = norm(coeff(1 : end - params.indexsetsize),'inf')/norm(squeeze(Q(:,n,:)));
end
t = toc;
...
fprintf('\n');
fprintf('Transform and inverse transform test -----------\n');

tic; 
for n = 1 : numvecs
    [coeff, levelcoeff, dcoeffs, ccoeffs] = hbtrans(sin(Q(:,n,:)), multileveltree, ind, datacell, datalevel);
    Qv = invhbtrans(dcoeffs, ccoeffs, multileveltree, ind, datacell, datalevel, numofpoints, dimdata);
    polyerror(n) = norm(squeeze(sin(Q(:,n,:))) - squeeze(Qv), 2) / norm(squeeze(Q(:,n,:)));
    wavecoeffnorm(n) = norm(coeff(1 : end - params.indexsetsize),'inf')/norm(squeeze(Q(:,n,:)));
end
t = toc;
