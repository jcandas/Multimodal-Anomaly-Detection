...
for n = numofnodes : -1 : 1 
    if isempty(transformcell{n,2}) == 0
        ...
        localfdata = ndmatrix(fdata(ixds,1,:));
        localcoeff = conj(localfdata') * W;
...
if isempty(transformcell{1,1}) == 0
    ...
    % ccoeffs = fdata' * V;
    % Need to be updated to take vectorial input data
    localfdata = ndmatrix(fdata);
    ccoeffs = conj(localfdata') * V;
    coeff = [coeff ccoeffs];
