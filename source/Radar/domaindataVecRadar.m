function [parameters,data] = domaindataVecRadar(methods,parameters,data);
...
rawdata = data.landsat.rawdata;
 testslice = squeeze(rawdata(:,:, parameters.data.testdata, modality : end));
%testslice = squeeze(data.landsat.rawdata(:,:,parameters.data.testdata,:));
...
% Debug: Work from here
% ---------------------------------------------------------------------

% Remove coordinate data (check, still not working)
datamask = ~isnan(data.landsat.rawdata(:,:, 1 : parameters.data.maxtime, modality : end));
