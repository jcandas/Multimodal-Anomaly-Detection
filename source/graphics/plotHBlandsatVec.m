...
h = figure(1);
subplot_tight(1,2,1,rawimagestight);
imagesc(maxtimesliceColor);
axis off
axis equal
title('Last training image', 'Interpreter', 'latex');

subplot_tight(1,2,2,rawimagestight);
imagesc(testsliceColor);
axis off
axis equal
title(['Testing image (day ',num2str(dias(parameters.data.testdata)),')'], 'Interpreter', 'latex');
...
for level = maxlevel : -1 : max(maxlevel - numlevel,0) 
    ...
    if parameters.graphics.print == true
        ...
        print(gcf, '-dpdf', '-bestfit', ['MB-CoeffLevels-',num2str(dias(parameters.data.testdata)),'.pdf']);
    end
...
set(h,'units','points','position',[x0,y0,width,height])
    ...
    if parameters.graphics.print == true
        ...
        'PaperPosition',[0 0 screenposition(3:4)],...
        'PaperSize',[screenposition(3:4)]);
        print(gcf, '-dpdf', '-bestfit', ['MB-Hypothesis-',num2str(dias(parameters.data.testdata)),'.pdf']);
    end
