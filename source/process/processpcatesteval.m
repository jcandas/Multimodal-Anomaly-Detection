function [parameters,data] = processpcatesteval(methods,parameters,data);
...
% Store data
data.landsat.rawtestslice = imgs;
...
len = size(imgs,3);
idx = 1:len;
gauss_height = gauss_heights;
trunc = zeros(len,1);
spe = zeros(len,1);
detect = zeros(len,1);

T = table(idx',gauss_height, trunc, spe, detect);
